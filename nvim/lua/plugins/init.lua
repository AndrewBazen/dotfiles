return {
    { import = "plugins.completions" },
    { import = "plugins.lazygit" },  -- Import the lazygit plugin configuration
    { import = "plugins.telescope" },  -- Import the telescope plugin configuration
    { import = "plugins.treesitter" },  -- Import the treesitter plugin configuration
    { import = "plugins.lsp-config" },  -- Import the LSP plugin configuration
    { import = "plugins.gitsigns" },  -- Import the comment plugin configuration
    { import = "plugins.vim-test" },  -- Import the indent-blankline plugin configuration
}